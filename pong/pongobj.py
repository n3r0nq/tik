...
class Plansza():
    ...
    def rysuj(self, *args):  # *args przekazana lista
        """ Rysowanie okna gry """
...
class PongGra(object):
    ...
    def __init__(self, szer, wys):
        ...
        self.pal1 = Paletka(szer=100, wys=20, x=350, y=360, kolor=(255, 0, 0))
        self.pal2 = Paletka(szer=100, wys=20, x=350, y=20, kolor=(0, 0, 255))
        self.pilka = Pilka(szer=20, wys=20, x=390,
                           y=190, kolor=(255, 105, 180))
        self.gracz2 = Gracz2(self.pal2, self.pilka, 2)
        # zegar
    ...
    def uruchom(self):
        ...
        while True:
            ...
            self.pilka.ruszaj(self.plansza.szer,
                              self.plansza.wys, self.pal1, self.pal2)
            self.gracz2.ruszaj()
            self.plansza.rysuj(
                self.pal1,
                self.pal2,
...
class ObiektGraf():
    """ Klasa bazowa dla rysowanych obiektow """
...
class Paletka(ObiektGraf):
    ...
    def przesun(self, mX, o_szer):  # mx ile pixeli w poziomie
        """
        :param:
        mX = skladowa X pozycji paletki
        o_szer = szerokosc okna gry
        """
...
class Pilka(ObiektGraf):
    ...
    def ruszaj(self, o_szer, o_wys, *args):
        # args lista obiektow z ktorymi moze wejsc w kolizje
        ...
        if self.prost.top <= 0 or self.prost.bottom >= o_wys:
            self.prost.x = (o_szer / 2) - 10
            self.prost.y = (o_wys / 2) - 10
